...
<?php if(get_user_total()){ ?>
	<div class="alert alert-info">
		<?php _e('Husk at prosjektene ikke er registret før du har sendt inn og betalt.', get_template()); ?>
	</div>
<?php } ?>
<div class="row-fluid">
	<div class="span4">
	...
	<?php while(have_posts()){the_post(); ?>
		...
		<div class="span4">
			<div class="index-project equalize">
				<a class="btn pull-right" href="<?php the_permalink(); ?>"><i class="icon-edit"></i> <?php _e('Rediger', get_template()); ?></a>
				<h4><?php  echo ucfirst($post->post_title); ?></h4>
				<div><?php echo implode(', ', wp_list_pluck(get_the_category('exclude=1'), 'name')); ?></div>
				<?php if($total = get_post_total()){ ?>
					<span class="label label-important"><?php echo sprintf(__('Utestående %s,- NOK', get_template()), $total); ?></span>
				<?php } ?>
			</div>
		</div>
	<?php } ?>
